...
| 2026-07-12 | **Historisch studentdossier — inzagescherm gemigreerde cijfers (opdrachtgever 2026-07-12).** Alleen-lezen overzichtsscherm voor de uit Access gemigreerde cijfers. **Lijst** (`historisch.index`): zoekbaar op studentnummer/naam, toont studenten met een historisch (BA-HIST) dossier + aantal gemigreerde cijfers. **Cijferlijst per student** (`historisch.show`): resultaten gegroepeerd per **studiejaar** (periode), met vakcode, vaknaam, eindcijfer (0–10), resultaat (voldoende/onvoldoende/vrijstelling) en behaalde EC; per jaar subtotaal EC + gemiddelde, en totaal EC behaald. Toegang **Examencommissie, Directie, Beheerder** — **niet** Studentenzaken (rolscheiding op cijfers, CLAUDE.md); inzage door Examencommissie/Directie wordt via `AuditLogger` gelogd. Sidebar-links onder Cijfers (EC/Dir) resp. Studenten (Beheer). Op de echte dev-data: **976 studenten** met dossier (bv. student 131516: 67 cijfers over 8 studiejaren). `HistorischDossierController` + views `historisch/index`,`historisch/show`; 5 tests (`HistorischDossierTest`). Handleidingen (medewerkers §7 + technisch) bijgewerkt. |
