...
MAIL_FROM_NAME="IUASR Studentenzaken"</span>
  ...
  <div class="let"><b>Dubbele inschrijving.</b> Collegegeld hangt aan het STUDIEJAAR, niet aan één inschrijving. <code>Collegegeldtermijnen::maatgevende()</code> kiest per (student, periode) de inschrijving met het hoogste verschuldigde bedrag (tie-break op laagste id); alleen die levert termijnen op. <code>voor()</code> geeft voor de andere inschrijving een lege collectie terug. Betalingen worden verrekend over <b>alle</b> inschrijvingen van dat studiejaar, en <code>BetalingController</code> boekt nieuwe betalingen (ook uit de CSV-import) altijd op de maatgevende. Reken de maatgevende nooit uit via het termijnschema zelf — gebruik <code>verschuldigdTotaal()</code>, anders ontstaat een kringverwijzing.</div>
