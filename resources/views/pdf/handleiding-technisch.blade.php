...
<body>
  ...
  <p>Aparte administratie, lichter regime (geen BSN/DUO). Tabellen: <code>cursussen</code> (code, naam, cursusgeld, start/eind, directeur_id, actief; drie cursussen in de create-migratie), <code>cursisten</code> (aangepaste, lichtere kopie van <code>studenten</code>; <code>cursistnummer</code> = C + jaar + volgnr via <code>CursistnummerGenerator</code>), <code>cursusinschrijvingen</code> (cursist × cursus, <code>totaalbedrag</code> als momentopname van het cursusgeld). Controllers onder <code>App\Http\Controllers\Cursus\*</code>, routes onder prefix <code>cursussen</code> met middleware <code>rol:cursusadministratie,beheerder</code>. Rol <code>Cursusadministratie</code> toegevoegd aan de enum (enum-kolom via ALTER). Bulk-import leest .xlsx én .csv via <code>App\Support\Tabellezer</code> (PhpSpreadsheet), kolomherkenning op naam. Cursusdirecteuren met toegangsbeperking en de cursusbetalingen volgen in latere fasen.</p>
