...
<div class="sis-card" style="margin-bottom:18px;">
  ...
  <div class="sis-choicegrid">
    ...
    <a class="sis-choice" href="{{ route('rapporten.alle-studenten') }}">
      ...
      <h4>Alle studenten — contactlijst (Excel)</h4>
      <p>De <b>hele database</b> aan studenten (elke status: actief, uitgeschreven, afgestudeerd). Alleen naam, telefoon en e-mail. Zonder IBAN en BSN.</p>
      <span class="tag">Excel · hele database · contactvelden</span>
    </a>
    @endif
    @if (auth()->user()->magInschrijvingBeheren())
    ...
    <a class="sis-choice" href="{{ route('ec-rapport') }}">
      ...
      <h4>EC-rapport</h4>
      <p>Studievoortgang per opleiding/klas: cumulatief behaalde studiepunten per student.</p>
    ...
    <div class="sis-choice" style="opacity:.55;">
      <span class="sis-choice__ic"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"/><rect x="7" y="10" width="3" height="8"/><rect x="12" y="6" width="3" height="12"/></svg></span>
      <h4>EC-rapport</h4>
      <p>Behaalde studiepunten per student en opleiding — cijfer-inzage vereist.</p>
      <span class="tag">geen cijferinzage voor deze rol</span>
    </div>
    @endif
    @if (auth()->user()->magCijfersInzien())
