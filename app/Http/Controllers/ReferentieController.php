        ...
        return [
            'opleidingen' => [
                ...
                'velden' => [
                    ...
                    'voldoende_grens' => ['label' => 'Voldoende-grens (cesuur)', 'type' => 'text', 'rules' => 'nullable|numeric|between:'.config('sis.cijfers.schaal_min', 1).','.config('sis.cijfers.schaal_max', 10), 'hint' => 'Leeg = systeemstandaard '.rtrim(rtrim(number_format((float) config('sis.cijfers.voldoende_grens_terugval'), 1), '0'), '.').'. TE BEVESTIGEN per OER'],
                    'ec_overgang_drempel' => ['label' => 'EC-drempel overgang (BSA)', 'type' => 'number', 'rules' => 'nullable|integer|min:0', 'hint' => 'Leeg = advies "onbekend". TE BEVESTIGEN per OER'],
                    'ec_model' => ['label' => 'EC-model', 'type' => 'select', 'opties' => ['knockout', 'compensatorisch'], 'leeg' => '— systeemstandaard ('.config('sis.cijfers.ec_model').') —', 'rules' => 'nullable|in:knockout,compensatorisch', 'hint' => 'knockout = elk onderdeel ≥ cesuur; compensatorisch = gewogen eindcijfer ≥ cesuur. TE BEVESTIGEN per OER'],
